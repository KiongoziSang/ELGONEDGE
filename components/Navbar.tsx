...
export function Navbar() {
  return (
    <header className="site-header sticky top-0 z-50 border-b border-slate-200/60 bg-white/82 shadow-sm shadow-slate-900/[0.03] backdrop-blur-2xl">
      <nav className="site-nav mx-auto flex max-w-[1520px] items-center justify-between gap-8 px-4 py-4 sm:px-6 lg:gap-10 lg:px-10 lg:py-5">
        <Link
          href="/"
          className="site-logo-link flex items-center rounded-[1.35rem] border border-white/70 bg-white/95 px-4 py-2.5 shadow-[0_16px_38px_rgba(15,23,42,0.10)] ring-1 ring-slate-900/[0.04] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_20px_44px_rgba(15,23,42,0.13)] sm:px-5 sm:py-3"
        ...
        >
          <Image
            ...
            height={92}
            priority
            className="site-logo h-12 w-auto object-contain sm:h-14 lg:h-16"
          />
        </Link>

        <div className="site-nav-pill hidden items-center gap-1.5 rounded-full border border-white/10 bg-gradient-to-b from-slate-800 via-brand-navy to-slate-950 p-2 shadow-[0_18px_45px_rgba(2,6,23,0.22)] ring-1 ring-cyan-200/10 backdrop-blur-xl xl:flex">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="site-nav-link group relative overflow-hidden rounded-full px-5 py-3 text-base font-black text-slate-300 transition duration-200 hover:-translate-y-0.5 hover:text-white"
            >
        ...
        <div className="site-nav-actions hidden items-center gap-3 xl:flex">
          <a
            href={companyProfilePdfPath}
            download
            className="site-profile-button inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-6 py-3.5 text-[0.95rem] font-black text-brand-navy shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-brand-blue/40 hover:text-brand-blue hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-brand-blue focus:ring-offset-2"
          >
            Profile
            <Download className="h-4 w-4" aria-hidden="true" />
          </a>
          <ButtonLink href={elgonOsUrl} variant="dark" className="site-launch-button gap-2 px-7 py-3.5">
            Launch ElgonOS
